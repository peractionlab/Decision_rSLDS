...
class SLDS(object):
    ...
    def _fit_laplace_em_discrete_state_update(
        ...
        # with open('cache/rslds_sample_x.txt', 'ab') as f:
        #     np.savetxt(f, x_sampless[0][0])
        ...
        if learning:
            ...
            lr = 0.005
            pi0 = variational_posterior.discrete_state_params[0]['pi0']
            Ps = variational_posterior.discrete_state_params[0]['Ps']
            log_likes = variational_posterior.discrete_state_params[0]['log_likes']
        ...
        for x_samples, data, input, mask, tag in \
            ...
            if learning:
                ...
                pi0 = lr * np.mean(
                    [self.init_state_distn.initial_state_distn
                    for x in x_samples], axis=0) + (1-lr) * pi0

                Ps = lr * np.mean(
                    [self.transitions.transition_matrices(x, input, x_mask, tag)
                    for x in x_samples], axis=0) + (1-lr) * Ps

                log_likes = lr * np.mean(
                    [self.dynamics.log_likelihoods(x, input, x_mask, tag)
                    for x in x_samples], axis=0) + (1-lr) * log_likes
    ...
    def _fit_laplace_em(self, variational_posterior, datas,
        ...
        for itr in pbar:
            # with open('cache/lds_z.txt', 'ab') as f:
            #     np.savetxt(f, variational_posterior.mean_discrete_states[0])
            # with open('cache/lds_x.txt', 'ab') as f:
            #     np.savetxt(f, variational_posterior.mean_continuous_states[0])
            # 1. Update the discrete state posterior q(z) if K>1
            if self.K > 1:
                self._fit_laplace_em_discrete_state_update(
                    variational_posterior, datas, inputs, masks, tags, num_samples, learning)
